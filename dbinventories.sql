...
DELIMITER //
    ...
    BEGIN
        DECLARE QUANTITYHISTORY INT;
        DECLARE ENTRYAMOUNT INT;
        DECLARE OUTPUTAMOUNT INT;
        DECLARE DEBIT FLOAT;
        DECLARE CREDIT FLOAT;
        DECLARE BALANCEEOA INT;
        DECLARE BALANCEDCA FLOAT;
        DECLARE LASTTOTALAMOUNT INT DEFAULT 0;
        DECLARE LASTTOTALVALUE FLOAT DEFAULT 0.0;
        SELECT COUNT(*) INTO QUANTITYHISTORY 
        FROM HISTORYINVENTORY 
        WHERE CODINV = _CODINV;
        IF QUANTITYHISTORY = 0 THEN
            SELECT TOTALAMOUNT, TOTALVALUEINV INTO ENTRYAMOUNT, DEBIT 
            FROM INVENTORY 
            WHERE CODEINV = _CODINV;
            SET OUTPUTAMOUNT = 0;
            SET CREDIT = 0;
            SET BALANCEEOA = 0;
            SET BALANCEDCA = 0;
        ELSE
            SELECT BALANCEDC, BALANCEEO, ENTRYAMOUNT, OUTPUTAMOUNT INTO BALANCEDCA, BALANCEEOA, LASTTOTALAMOUNT, LASTTOTALVALUE
            FROM HISTORYINVENTORY 
            WHERE CODINV = _CODINV 
            ORDER BY CODEHISTORYINV DESC 
            LIMIT 1;
            SELECT TOTALAMOUNT, TOTALVALUEINV INTO ENTRYAMOUNT, DEBIT 
            FROM INVENTORY 
            WHERE CODEINV = _CODINV;
            IF ENTRYAMOUNT < LASTTOTALAMOUNT THEN
                SET OUTPUTAMOUNT = LASTTOTALAMOUNT - ENTRYAMOUNT;
                SET ENTRYAMOUNT = 0;
            ELSE
                SET OUTPUTAMOUNT = 0;
                SET ENTRYAMOUNT = ENTRYAMOUNT - LASTTOTALAMOUNT;
            END IF;
            IF DEBIT < LASTTOTALVALUE THEN
                SET CREDIT = LASTTOTALVALUE - DEBIT;
                SET DEBIT = 0;
            ELSE
                SET CREDIT = 0;
                SET DEBIT = DEBIT - LASTTOTALVALUE;
            END IF;
        END IF;
        SET BALANCEEOA = BALANCEEOA + ENTRYAMOUNT - OUTPUTAMOUNT;
        SET BALANCEDCA = BALANCEDCA + DEBIT - CREDIT;
        INSERT INTO HISTORYINVENTORY (DATEQUERY, DETAIL, EXITDOCUMENT, ENTRYAMOUNT, OUTPUTAMOUNT, BALANCEEO, DEBIT, CREDIT, BALANCEDC, CODINV) 
        VALUES (CURRENT_DATE, _DETAIL, _EXITDOCUMENT, ENTRYAMOUNT, OUTPUTAMOUNT, BALANCEEOA, DEBIT, CREDIT, BALANCEDCA, _CODINV);
    END //
DELIMITER ;

DELIMITER //
    CREATE PROCEDURE CREATEHISTORYPRODUCT(IN _DETAIL VARCHAR(128), IN _EXITDOCUMENT VARCHAR(32), IN _CODPROD INT)
    BEGIN
        DECLARE QUANTITYHISTORY INT;
        DECLARE ENTRYAMOUNT INT;
        DECLARE OUTPUTAMOUNT INT;
        ...
        IF QUANTITYHISTORY = 0 THEN
            SELECT TOTALAMOUNT, TOTALVALUE INTO ENTRYAMOUNT, DEBIT 
            FROM PRODUCT 
            WHERE CODEPROD = _CODPROD;
            SET OUTPUTAMOUNT = 0;
            SET CREDIT = 0;
            SET BALANCEEOA = 0;
            SET BALANCEDCA = 0;
        ...
        SET BALANCEEOA = BALANCEEOA + ENTRYAMOUNT - OUTPUTAMOUNT;
        SET BALANCEDCA = BALANCEDCA + DEBIT - CREDIT;
        INSERT INTO HISTORYPRODUCT (DATEQUERY, DETAIL, EXITDOCUMENT, ENTRYAMOUNT, OUTPUTAMOUNT, BALANCEEO, DEBIT, CREDIT, BALANCEDC, CODPROD) 
        VALUES (CURRENT_DATE, _DETAIL, _EXITDOCUMENT, ENTRYAMOUNT, OUTPUTAMOUNT, BALANCEEOA, DEBIT, CREDIT, BALANCEDCA, _CODPROD);
    END //
DELIMITER ;
